...
class RsSpider(scrapy.Spider):
    ...
    custom_settings = {
        "FEEDS": {
            "paroquias_rs_final.jsonl": {"format": "jsonlines", "encoding": "utf-8"}
        }
    }
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(
            url="https://www.arquipoa.com/paroquias", callback=self.make_city_requests
        )
    ...
    def parse(self, response):
        ...
        for parish_div in response.css("div.col-12.col-sm-6.col-md-8"):
            ...
            # Extrai telefone
            phone_raw = (
                parish_div.css('p:nth-of-type(2) a[href^="tel:"]::text').get() or ""
            )
            phone = phone_raw.replace(" ", "").replace("-", "") if phone_raw else ""

            # Extrai email
            email_match = parish_div.css("p:nth-of-type(2)").re(
                r"[\w\.-]+@[\w\.-]+\.[\w]+"
            )
            ...
            if whatsapp_link:
                ...
                if whatsapp_match:
                    ...
                    if len(whatsapp_number) in [10, 11]:
                        ...
                        whatsapp = (
                            f"+{whatsapp_number}"  # Números já com código do país
                        )
                    else:
            ...
            for p in paragraphs:
                ...
                if found_schedule_header and p_text:
                    ...
                    if text and (
                        ...
                        word in text.lower()
                        for word in ["tel", "telefone", "@", "email", "endereço"]
                    ):
