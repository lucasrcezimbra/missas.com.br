...
source, _ = Source.objects.get_or_create(
    type=Source.Type.SITE, description="Site da Arquidiocese de Porto Alegre"
)
...
for d in datas:
    ...
    parish, created = Parish.objects.get_or_create(
        city=city, name=parish_name, defaults={"slug": unique_slug}
    )
    ...
    for schedule_text in d.get("schedules", []):
        ...
        if not found_days:
            ...
            if "terça" in schedule_text.lower() and "sábado" in schedule_text.lower():
                ...
                "segunda" in schedule_text.lower() and "sexta" in schedule_text.lower()
            ):
