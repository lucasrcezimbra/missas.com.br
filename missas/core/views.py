...
def nearby_schedules(request):
    ...
    type_name = request.GET.get("tipo")
    verified_only = request.GET.get("verificado") == "1"
    max_distance = request.GET.get("distancia", "10")
    ...
    for schedule in schedules:
        distance = haversine_distance(
            user_lat, user_lon, schedule.location.latitude, schedule.location.longitude
        )
        if distance <= max_distance:
            schedule.distance = distance
            schedules_with_distance.append(schedule)

    schedules_with_distance.sort(key=lambda s: (s.distance, s.day, s.start_time))
